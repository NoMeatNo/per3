...
class FarsiFlix3Provider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    override val mainPage = mainPageOf(
        "$mainUrl/iran-movie-2025/" to "Movies",
        "$mainUrl/series-26/" to "Last Series",
        "$mainUrl/iranian-series/" to "Old Series",
        "$mainUrl/old-iranian-movies/" to "Old Movies",
        "$mainUrl/episodes-15/" to "Last Episodes",
...
override suspend fun getMainPage(
    ...
    val link = when (request.name) {
        "Movies" -> "$mainUrl/iran-movie-2025/"
        "Last Series" -> "$mainUrl/series-26/"
        "Old Series" -> "$mainUrl/iranian-series/"
        "Old Movies" -> "$mainUrl/old-iranian-movies/"
        "Last Episodes" -> "$mainUrl/episodes-15/"
    ...
    return try {
        ...
        if (qualityForms.isNotEmpty()) {
            ...
            qualityForms.forEach { form ->
                ...
                val action = fixUrl(form.attr("action")) // Ensure URL is absolute
                val postIdForRedirect = form.selectFirst("input[name=postid]")?.attr("value") ?: return@forEach

                try {
                    // Get final redirect page URL to use as referer
                    ...
                    if (mp4Link.isNotBlank()) {
                        // Set quality based on 'q' parameter
                        val quality = when (q) {
                        ...
                        callback.invoke(
                            newExtractorLink(
                                ...
                                // Critical: Add referer from the form action URL
                                this.referer = action
